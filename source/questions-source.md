...
### Day 21 — 汇川一面实战复盘原题（2026-07-16 晚）

> 汇川技术 Java 一面当天真实被问到、没答上来的 4 道题。与 Day 20（考前预测 84 题）的区别：这些是实战真正被问到的题，优先复盘。完整深度答案见 Wiki `06-前端基础/` 与 `03-Java后端/分布式与微服务/`。

#### 简答题

- `[D21-01]` 前端组件间通信方式有哪些？
  > 按组件关系分：① 父子：props 向下传、$emit 向上报（单向数据流，子不能直接改 prop）；② 兄弟：状态提升到共同父级，或事件总线 EventBus；③ 跨层级：provide/inject 依赖注入（React 用 Context），避免层层透传 props；④ 全局状态：Pinia/Vuex（React 用 Redux/Zustand）适合登录态、购物车；⑤ ref 直接拿实例。坑：EventBus 组件销毁要 $off 解绑否则内存泄漏，Vue3 已移除实例上的 $on。
- `[D21-02]` 前端路由有几种方式？说说原理。
  > 前端路由本质是 URL 与视图同步且不发服务器请求。两种模式：① hash——URL 带 #，监听 hashchange 事件，# 后内容不发请求，兼容好但 URL 丑、SEO 差；② history——URL 干净，用 H5 的 pushState/replaceState 改地址 + popstate 处理前进后退。关键坑：history 刷新会向服务器请求该路径→404，必须 nginx 配 try_files $uri $uri/ /index.html 回退到 index.html；hash 刷新安全不用配。
- `[D21-03]` SpringCloud 微服务架构下，接口慢如何做全链路分析？
  > 三步走：① 定位——用 traceId + SkyWalking/Zipkin 看 trace 链路，找耗时最长的 span（网关→认证→业务→Feign 调下游→DB/Redis/MQ）；② 分层排查——应用层查 N+1 查询/锁竞争/线程池打满/频繁 Full GC（jstat + jmap dump + MAT），数据层查慢 SQL（慢查询日志 + EXPLAIN 看 type/key/Extra）/锁等待，RPC 层查调下游耗时 + 超时重试放大（需熔断）；③ 工具——SkyWalking 链路、Prometheus+Grafana 监控、Arthas+jstack 在线诊断、EXPLAIN。实战：MES 循环查主数据的 N+1 改批量查询，RT 从 2s 降到 200ms。
- `[D21-04]` RocketMQ 为什么能实现高并发？
  > 四大支柱：① 顺序写磁盘——所有消息追加写 CommitLog，磁盘顺序写接近内存速度；且所有 topic 共用一个 CommitLog，topic 多也稳定（强于 Kafka 每个 partition 独立文件、多 topic 退化为随机写）；② PageCache + 零拷贝 mmap——写先进页缓存异步刷盘，读命中页缓存，mmap 内存映射省掉内核到用户态拷贝；③ 存储结构——CommitLog 顺序写存物理消息，ConsumeQueue 做逻辑索引存偏移量，写顺序、读靠索引；④ Netty NIO Reactor 全异步 + 高可用（Master-Slave / Dledger 基于 Raft 自动选主 / NameServer 无状态 AP）。对比：RocketMQ 用 mmap、Kafka 用 sendfile；RocketMQ 适合业务消息（事务/延迟/顺序），Kafka 吞吐更高适合大数据。
